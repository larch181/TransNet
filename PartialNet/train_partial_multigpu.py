...
parser.add_argument('--num_gpus', type=int, default=8, help='How many gpus to use [default: 1]')
                ...
                with tf.variable_scope(tf.get_variable_scope(), reuse=bool(i != 0)):
                    with tf.device('/gpu:%d' % (i)), tf.name_scope('gpu_%d' % (i)) as scope:
                        ...
                        input_gt_small_batch = tf.slice(pointclouds_gt_small,
                                                        [i * DEVICE_BATCH_SIZE, 0, 0], [DEVICE_BATCH_SIZE, -1, -1])
                        pred_result = MODEL.get_model(input_pc_batch, is_training=is_training_pl, bn_decay=bn_decay)
                        cd_dists_g, knn_dists_g = loss_util.get_partialNet_loss(pred_result, input_angle_batch,
                                                                                input_pc_batch,
                                                                                input_gt_batch, input_gt_big_batch,
                                                                                input_gt_small_batch)
