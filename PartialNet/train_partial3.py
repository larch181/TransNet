...
parser.add_argument('--log_dir', default='log/PartialNet15/', help='Log dir [default: log]')
...
TRAIN_DATASET = PartialDataPatcher.TransNetH5Dataset(
    os.path.join(BASE_DIR, 'data/train_files_pwd.txt'), batch_size=BATCH_SIZE,
    npoints=NUM_POINT, shuffle=True)
TEST_DATASET = PartialDataPatcher.TransNetH5Dataset(
    os.path.join(BASE_DIR, 'data/test_files_pwd.txt'), batch_size=BATCH_SIZE,
...
def load_gt_xyz_file(batch_size, num_point, filename=None):
    if filename == None:
        filename = os.path.join(POINT_CLOUD_FOLDER, 'Dataset/pwd/pwd_gt_20480.xyz')
    print(filename)
    data_T = np.loadtxt(filename)
    data = data_T[:, 0:3]
    pointcloud_gt = np.repeat(data[np.newaxis, ...], batch_size, axis=0)

    filename = os.path.join(POINT_CLOUD_FOLDER, 'Dataset/pwd/pwd_gt_4096.xyz')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            cd_dists, knn_dists = loss_util.get_partialNet_loss(pred_angle, pointclouds_angle, pointclouds_pl,pointclouds_gt)
