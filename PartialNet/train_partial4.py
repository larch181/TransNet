...
parser.add_argument('--log_dir', default='log/PartialNet18/', help='Log dir [default: log]')
...
def load_gt_xyz_file(batch_size, num_point, filename=None):
    if filename == None:
        filename = os.path.join(POINT_CLOUD_FOLDER, 'Dataset/einstein/einstein_gt_20480.xyz')
    print(filename)
    data_T = np.loadtxt(filename)
    data = data_T[:, 0:3]
    pointcloud_gt_big = np.repeat(data[np.newaxis, ...], batch_size, axis=0)

    filename = os.path.join(POINT_CLOUD_FOLDER, 'Dataset/einstein/einstein_gt_4096.xyz')
    data_S = np.loadtxt(filename)
    data2 = data_S[:, 0:3]
    pointcloud_gt = np.repeat(data2[np.newaxis, ...], batch_size, axis=0)

    return pointcloud_gt_big, pointcloud_gt


pointcloud_gt_big_val, pointcloud_gt_val = load_gt_xyz_file(BATCH_SIZE, NUM_POINT)

print(pointcloud_gt_big_val.shape[1] / pointcloud_gt_val.shape[1])
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            pointclouds_pl, pointclouds_angle, pointclouds_gt,pointclouds_gt_big = MODEL.placeholder_inputs(BATCH_SIZE, NUM_POINT)
            ...
            # Get model and loss 
            pred_angle = MODEL.get_model2(pointclouds_pl,pointclouds_gt, is_training_pl, bn_decay=bn_decay)

            # loss_util.get_transform_loss2(pred_angle, pointclouds_angle)
            cd_dists, knn_dists = loss_util.get_partialNet_loss(pred_angle, pointclouds_angle, pointclouds_pl,pointclouds_gt_big)
