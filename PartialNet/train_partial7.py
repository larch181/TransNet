...
parser.add_argument('--log_dir', default='log/PartialNetNew3/', help='Log dir [default: log]')
...
parser.add_argument('--result_dim', type=int, default=7, help='Dimension of result(quaternion:4, rotation: 3) [default: 4]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            pointclouds_pl, pointclouds_angle, pointclouds_gt,pointclouds_gt_big = MODEL.placeholder_inputs(BATCH_SIZE, NUM_POINT,CD_Ratio,K=K)
            ...
            # Get model and loss
            pred_angle = MODEL.get_model_trans(pointclouds_pl,is_training_pl, bn_decay=bn_decay,K=K)
        ...
        for epoch in range(MAX_EPOCH):
            ...
            train_one_epoch(sess, ops, train_writer,K=K)
            ...
            if epoch % 50 == 0:
                eval_one_epoch(sess, ops, test_writer,K=K)

def train_one_epoch(sess, ops,train_writer,K=7):
    ...
    cur_batch_angle = np.zeros((BATCH_SIZE, K))
    ...
    while TRAIN_DATASET.has_next_batch():
        ...
        cur_batch_angle[0:bsize, ...] = batch_angle[:,:K]
...
def eval_one_epoch(sess, ops, test_writer,K=7):
    is_training = False
    ...
    while TEST_DATASET.has_next_batch():
        ...
        # for the last batch in the epoch, the bsize:end are from last batch
        cur_batch_data[0:bsize, ...] = batch_data
        cur_batch_angle[0:bsize, ...] = batch_angel[:,:K]
        feed_dict = {ops['pointclouds_pl']   : cur_batch_data,
                     ops['pointclouds_gt']    : pointcloud_gt_val,
                     ops['pointclouds_gt_big']: pointcloud_gt_big_val,
                     ops['pointclouds_angle']: cur_batch_angle,
        ...
        transform_xyz_input = Tools3D.batch_quaternion2mat(cur_batch_angle[:,:4])

        transform_xyz = Tools3D.batch_quaternion2mat(pred_angle[:,:4])
