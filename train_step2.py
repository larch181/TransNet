...
parser.add_argument('--log_dir', default='log/PartialNet3/', help='Log dir [default: log]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            _, cd_dist = loss_util.get_transformnet_loss_quaternion(pred_angle, pointclouds_angle, pointclouds_pl,
                                                                     pointclouds_gt)
        ...
        for epoch in range(restore_epoch,MAX_EPOCH):
            ...
            if epoch % 10 == 0:
                ...
                save_path = saver.save(sess, os.path.join(LOG_DIR, "model"), global_step=epoch)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    while TRAIN_DATASET.has_next_batch():
        ...
        summary, step, _, loss_val,cd_dist = sess.run([ops['merged'], ops['step'],
                                               ops['train_op'], ops['loss'],ops['cd_dist']],
                                              feed_dict=feed_dict)
...
def eval_one_epoch(sess, ops,test_writer):
    ...
    while TEST_DATASET.has_next_batch():
        batch_data, batch_angel, batch_data_label = TEST_DATASET.next_batch(augment=True)
        bsize = batch_data.shape[0]
        ...
        summary, step,loss_val, pred_angle, cd_dist = sess.run([ops['merged'], ops['step'],ops['loss'], ops['pred_angle'], ops['cd_dist']],
                                                  feed_dict=feed_dict)
        ...
        for i in range(bsize):
            img_filename = '%d.png' % (i+(batch_idx-1)*BATCH_SIZE)#, datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            ...
            align_loss = cd_dist[i]  # , _ = loss_util.get_emd_loss2(point_cloud_transformed[i,:,:], cur_batch_data[i, :, :])
            ...
            loss_emd = pc_util.log_visu_loss('CD  Loss', align_loss)
