...
class RotationPredictor:
    def __init__(self,model_name='einstein'):
        ...
        filename_big = os.path.join(POINTCLOUD_DIR, 'Dataset/%s/%s_gt_20480.xyz'%(model_name,model_name))
        filename_small = os.path.join(POINTCLOUD_DIR, 'Dataset/%s/%s_gt_1024.xyz'%(model_name,model_name))
        self.pointcloud_gt = np.loadtxt(filename)
        self.pointcloud_gt_big =np.loadtxt(filename_big)
        self.pointcloud_gt_small =np.loadtxt(filename_small)
        self.init_network()
        self.startime = time()
        self.predict_trans = None#np.eye(3)
    def init_network(self):
        self.model_path = os.path.join(ROOT_DIR,'log/PartialNetNew4/model.ckpt')
        ...
        with tf.Graph().as_default():
            with tf.device('/gpu:' + str(0)):
                ...
                pointclouds_gt_small = tf.placeholder(tf.float32, shape=(1, None, 3))
                is_training_pl = tf.placeholder(tf.bool, shape=())
                pred_angle = PartialNet.get_model(pointclouds_pl, is_training_pl,bn_decay=0.5)
            ...
            restore_epoch, checkpoint_path = model_utils.pre_load_checkpoint(os.path.dirname(self.model_path))
             # Restore variables from disk.
            model_name = checkpoint_path.split('/')[-1]
            checkpoint_path = os.path.join(os.path.dirname(self.model_path),model_name)
